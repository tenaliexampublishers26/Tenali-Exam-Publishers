...
export async function POST(req: Request) {
  try {
    ...
    if (body.settings && typeof body.settings === 'object') {
      ...
      if (body.settings.maintenance_mode !== undefined) {
        const isMaint = body.settings.maintenance_mode === true || body.settings.maintenance_mode === 'true';
        response.cookies.set('tep_maintenance', isMaint ? 'true' : 'false', {
          path: '/',
          maxAge: 31536000,
          sameSite: 'lax',
        });
