 ...
 * ──────────────────────────────────
 * Previously 'use client' — which meant:
 *  ❌ Full JS bundle downloaded before first paint
 *  ❌ Products fetched AFTER page load (waterfall)
 *  ❌ No server-side HTML — bad for SEO and slow on mobile
 *
 * Now server-rendered:
 *  ✅ HTML sent to browser instantly (streaming)
 *  ✅ Products pre-fetched on the server — zero client waterfall
 *  ✅ Only the scroll buttons are client-side (thin HeroActions wrapper)
 *  ✅ Works with Next.js route caching (revalidate = 30)
 */
...
import HeroActions from './HomePageClient';
import styles from './page.module.css';

// ─── Route-level ISR: Next.js revalidates this page every 30 seconds ──────
...
// Runs on the SERVER — users never wait for a client-side fetch waterfall.
// Cached with stale-while-revalidate so only 1 DB query runs per 30s window.
async function getProducts(): Promise<Product[]> {
  try {
    const products = await withServerCache<Product[]>(
      'homepage-products',
      async () => {
      ...
      { ttl: 30_000, tags: ['products'], staleWhileRevalidate: true }
    );
    return products;
  } catch {
    return [];
...
// ─── Product Grid (async sub-component using Suspense) ─────────────────────
...
export default function HomePage() {
  return (
    <>
      ...
      <section className={styles.hero}>
        <div className={`container ${styles.heroInner}`}>
          ...
          <div className={styles.heroContent}>
            ...
            {/* HeroActions is the ONLY client component on this page */}
            <HeroActions />
          ...
          <div className={styles.heroVisual}>
            <div className={styles.heroBannerContainer}>
              ...
              {/* Next.js <Image> instead of plain <img>:
          ...
          <div className={styles.highlightsDesktopGrid}>
            {HIGHLIGHTS.map((item, idx) => (
              <div key={idx} className={styles.highlightCard}>
                <div className={styles.highlightIconWrap}>{item.icon}</div>
            ...
            {EXAM_CATEGORIES.map((cat, idx) => (
              <Link href={`/study-materials?category=${encodeURIComponent(cat.name)}`} key={idx} className={styles.categoryCard}>
                <div className={styles.categoryCardHeader}>
                  <div className={styles.categoryIconWrap}>{cat.icon}</div>
      ...
      {/* Featured Books — Suspense streams skeleton while server fetches products */}
      <section id="books" className={`${styles.section} ${styles.sectionAlt}`}>
        <div className="container">
          ...
          <div className={styles.featuredGrid}>
            {/* Suspense: shows skeletons instantly, swaps in real cards when server fetch completes */}
